const FirebaseService = {
  ...
  async uploadDocument(section, id, file, onProgress = () => {}) {
    ...
    return new Promise((resolve, reject) => {
      // contentDisposition: 'inline' evita que el navegador fuerce la descarga del
      // archivo en vez de mostrarlo dentro de la app (visor de PDF, imágenes, etc.)
      const task = ref.put(file, { contentType, contentDisposition: 'inline' });
  ...
  // Reparar en segundo plano documentos subidos antes de activar contentDisposition:'inline',
  // para que dejen de forzar la descarga y se puedan previsualizar dentro de la app.
  repairInlineDisposition(path) {
    if (!path || !this.hasCredentials()) return;
    const ref = this._storage.ref(path);
    ref.getMetadata().then((meta) => {
      if (meta.contentDisposition !== 'inline') {
        return ref.updateMetadata({ contentDisposition: 'inline' });
      }
    }).catch(() => {});
  },
