 ...
 * Manejo del estado local, interfaz de usuario (UI), eventos y persistencia en Firebase / LocalStorage
 */
...
const app = {
  ...
  async init() {
    ...
    // Esperar a conocer el estado de sesión inicial antes de decidir qué mostrar
  ...
  // Cargar (o recargar) los datos de la app. Se llama tras iniciar sesión o registrarse.
  async loadAppData() {
    this.showLoading("Cargando tus datos...");
    ...
    const remoteDb = await FirebaseService.loadDb();
    ...
    if (hasLocalData && isRemoteEmpty) {
      ...
      this.db = localDb;
      // Intentar subir a Firebase para sincronizar
      if (FirebaseService.hasCredentials()) {
        try {
          await FirebaseService.saveDb(this.db);
        } catch (e) {
          console.error("Error al sincronizar datos locales a Firebase durante la inicialización:", e);
        }
      }
    } else {
    ...
    // 3b. Reparar en segundo plano documentos antiguos que aún se descargan en vez de previsualizarse
    this._repairLegacyDocuments();

    // 4. Ocultar Loading
    this.hideLoading();
  },

  // Recorrer todos los documentos guardados y reparar (en segundo plano, sin bloquear la UI)
  // los que se subieron antes de activar la previsualización en línea.
  _repairLegacyDocuments() {
    const paths = [];

    Object.values(this.db.temario || {}).forEach(t => {
      (t.archivos || []).forEach(a => a && a.path && paths.push(a.path));
    });

    Object.values(this.db.calendario || {}).forEach(d => {
      (d.archivos || []).forEach(a => a && a.path && paths.push(a.path));
      (d.transcripciones || []).forEach(t => t && t.path && paths.push(t.path));
    });

    (this.db.documentosTest || []).forEach(doc => doc && doc.path && paths.push(doc.path));

    paths.forEach(path => FirebaseService.repairInlineDisposition(path));
  },
  ...
  async handleLogout() {
    if (!confirm("¿Seguro que quieres cerrar sesión? Podrás volver a entrar con tu email y contraseña en cualquier dispositivo.")) return;
  ...
  async handleForgotPassword() {
    const email = document.getElementById('auth-email').value.trim();
    if (!email) {
      alert("Escribe primero tu email en el campo de arriba y pulsa de nuevo el enlace.");
      return;
    }
    try {
  ...
  _validateAuthForm(email, password) {
    if (!email || !password) {
  ...
  _translateAuthError(e) {
    ...
    const map = {
      ...
      'auth/user-not-found': 'No existe ninguna cuenta con ese email. Pulsa "Crear Cuenta" si es tu primera vez.',
  ...
  updateSyncStatusUI() {
    ...
    if (FirebaseService.hasCredentials()) {
      ...
      label.innerText = "Firebase Conectado";
    } else {
  ...
  async syncWithFirebase() {
    ...
    this.showLoading("Sincronizando progreso con Firebase...");
    try {
      // 1. Guardar local a remoto
      await FirebaseService.saveDb(this.db);
      // 2. Volver a cargar para asegurarnos que tenemos lo último
      this.db = await FirebaseService.loadDb();
      ...
      alert("¡Sincronización completada! Todos tus datos están guardados en Firebase.");
    } catch (e) {
    ...
    archivos.forEach((file, index) => {
      ...
      item.innerHTML = `
        ...
        <div class="doc-actions">
          <button class="btn-icon" onclick="app.previewDocument('${file.url}', '${file.name.replace(/'/g, "\\'")}', '${file.type}', '${file.path}')" title="Ver en la App">
  ...
  async handleTemaDocUpload(event) {
    ...
    try {
      ...
      const uploadedFile = await FirebaseService.uploadDocument('temario', `tema_${this.selectedTemaId}`, file);
  ...
  async handleTemaDocDelete(index) {
    ...
    try {
      const res = await FirebaseService.fetchFile(fileToDelete.path, false);
      if (res && res.sha) {
        await FirebaseService.deleteFile(fileToDelete.path, res.sha);
      }
  ...
  async handleCalendarDocUpload(event) {
    ...
    try {
      const uploadedFile = await FirebaseService.uploadDocument('calendario', this.selectedDate, file);
  ...
  // incrustar sus URLs directamente dentro de un <iframe> de otro dominio (X-Frame-Options).
  async previewDocument(url, name, type, path) {
    ...
    if (resolvedType.startsWith('image/')) {
      content.innerHTML = `<img src="${url}" style="max-width:98%; max-height:98%; object-fit:contain; border-radius:8px; box-shadow:0 8px 30px rgba(0,0,0,0.5);">`;
    } else if (resolvedType === 'application/pdf') {
      // Detectar si es iOS (iPhone/iPad/iPod) para esquivar la limitación de Safari de no incrustar PDFs en iframes
      ...
      if (isIOS) {
        content.innerHTML = `
          <div style="color:#fff; padding:32px 16px; text-align:center; display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; gap:20px; background:#040811; border-radius:8px;">
            ...
            <a href="${url}" target="_blank" class="btn btn-primary" style="padding:12px 24px; font-size:13px; text-decoration:none; display:inline-flex; align-items:center; gap:8px; border-radius:6px; font-weight:600; cursor:pointer;">
        ...
        content.innerHTML = `
          <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:50px; gap:16px; color:#fff; height:100%;">
            ...
            <p style="font-size:13px; color:var(--text-muted);">Cargando visor de PDF...</p>
          </div>
        `;
        try {
          let bytes = null;
  ...
  async handleCalendarNotesTranscription(event) {
    ...
    if (!FirebaseService.hasCredentials()) {
      alert("Debes iniciar sesión para guardar las fotos de tus apuntes.");
      document.getElementById('auth-gate-overlay').style.display = 'flex';
      return;
    }

    this.showLoading("Subiendo imagen de apuntes y transcribiendo con Gemini AI...");
    ...
    try {
      ...
      const uploadedFile = await FirebaseService.uploadDocument('calendario', `${this.selectedDate}/transcripciones`, file);
      ...
      card.innerHTML = `
        ...
        <div style="flex: 1; display: flex; flex-direction: column; gap: 8px; overflow: hidden;">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size:12px; font-weight:600; color:var(--color-accent); cursor:pointer;" onclick="app.previewDocument('${t.imageUrl}', '${t.filename.replace(/'/g, "\\'")}', 'image/png', '${t.path}')" title="Ampliar imagen">
              📷 Ampliar Imagen
            </span>
          </div>
  ...
  async deleteCalendarTranscription(index) {
    if (!confirm("¿Seguro que deseas eliminar esta transcripción y su foto?")) return;

    const transcripciones = this.db.calendario[this.selectedDate].transcripciones;
    const itemToDelete = transcripciones[index];

    this.showLoading("Eliminando foto y datos de transcripción...");

    try {
      // 1. Eliminar archivo de Firebase Storage
      const res = await FirebaseService.fetchFile(itemToDelete.path, false);
      if (res && res.sha) {
        await FirebaseService.deleteFile(itemToDelete.path, res.sha);
      }
